...
(function($ , undefined) {
	...
	var preview_image = function(file) {
		...
		reader.onload = function (e) {
			$span.prepend("<personal-img class='middle' style='display:none;' />");
			var img = $span.find('personal-img:last').get(0);
	...
	var get_thumbnail = function(img, size, type) {
		...
		//**IE10** is not giving correct width using personal-img.width so we use $(personal-img).width()
		w = w > 0 ? w : $(img).width()
		h = h > 0 ? h : $(img).height()
