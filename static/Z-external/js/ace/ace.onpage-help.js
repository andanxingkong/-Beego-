jQuery(function($) {
	...
	var get_section_url = function(section_name) {
		...
		var parts = url.split('/');
		if(parts.length == 1) {
			//for example convert `changes` to `changes/personalpage.html`
			if(url.length == 0) url = 'intro';//or convert `empty string` to `intro/personalpage.html`
			url = url + '/personalpage.html';
		}
