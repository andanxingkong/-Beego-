...
type RoadSegmentlist struct {
	beego.Controller
}

func (c *RoadSegmentlist) Get() {
	if !controllers.IsUserLoggedIn(&c.Controller) {
		// 用户未登录、返回错误信息或者跳转到登录页面
		c.Redirect("/login", 302)
		return
	}
	c.TplName = "RoadSegment/RoadSegmentlist.html"
}

func (c *RoadSegmentlist) GetAll() {
	o := orm.NewOrm()
	var roadSegment []models.RoadSegment
	o.QueryTable("road_segment").All(&roadSegment)
	if len(roadSegment) > 0 { //查询到了用户数据
		var respList []interface{}
		for _, roadSegment := range roadSegment {
			respList = append(respList, roadSegment.RoadSegmentToRespDesc())
		}
...
func (c *RoadSegmentlist) DeleteRoadSegment() {
	...
	_, err = o.Delete(&models.RoadSegment{Road_id: id})
...
func (c *RoadSegmentlist) FindById() {
	id, _ := c.GetInt(":id")
	var roadSegment models.RoadSegment
	o := orm.NewOrm()
	error := o.QueryTable("road_segment").Filter("Road_id", id).One(&roadSegment)
	if error != nil {
		// 查询出错
	...
	// 将用户信息以JSON格式返回给前端
	c.Data["json"] = roadSegment
	c.ServeJSON()
}

func (c *RoadSegmentlist) FindBySome() {
	...
	var RoadSegments []models.RoadSegment
	o := orm.NewOrm()
	o.QueryTable("road_segment").Filter("Name__contains", keyword.Mohu).All(&RoadSegments)
	fmt.Println("=====================", RoadSegments)
	var respList []interface{}
	for _, RoadSegment := range RoadSegments {
		respList = append(respList, RoadSegment.RoadSegmentToRespDesc())
	}
