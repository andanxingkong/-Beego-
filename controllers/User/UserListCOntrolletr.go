...
type UesrListController struct {
	beego.Controller
}

func (c *UesrListController) GetAll() {
	o := orm.NewOrm()
	var users []models.Users
	o.QueryTable("Users").All(&users)
	if len(users) > 0 { //查询到了用户数据
...
func (c *UesrListController) DeleteUser() {
...
func (c *UesrListController) FindById() {
	id, _ := c.GetInt(":id")
	var user models.Users
	o := orm.NewOrm()
	error := o.QueryTable("Users").Filter("id", id).One(&user)
	if error != nil {
		// 查询出错
...
func (c *UesrListController) FindBySome() {
	...
	var users []models.Users
	o := orm.NewOrm()
	o.QueryTable("Users").Filter("Username__contains", keyword.Mohu).All(&users)
	fmt.Println("=====================", users)
	var respList []interface{}
	for _, user := range users {
		respList = append(respList, user.UserToRespDesc())
